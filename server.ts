...
import foodRoutes from './server/routes/foodRoutes.ts';
import adminRoutes from './server/routes/adminRoutes.ts';
import { User, UserRole, UserStatus } from './server/db.ts';
import { createServer as createViteServer } from 'vite';
...
if (MONGODB_URI) {
  mongoose.connect(MONGODB_URI, {
    ...
    .then(async () => {
      ...
      // Admin Seeder logic
      try {
        const adminEmail = 'joyboy@gmail.com';
        ...
        if (!existingAdmin) {
          ...
          console.log('✅ Admin account created: joyboy@gmail.com / admin123');
        } else {
          console.log('ℹ️ Admin account already exists');
        }
      } catch (error) {
        console.error('❌ Admin seeding failed:', (error as any).message);
      }
    })
    .catch((err) => {
