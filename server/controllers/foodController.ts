...
import { FoodRequest, FoodRequestStatus, User } from '../db.ts';
...
export const getDonorRequests = async (req: Request, res: Response) => {
  try {
    const donorId = (req as any).user.userId;
...
export const acceptRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (request.status && request.status !== FoodRequestStatus.PENDING) return res.status(400).json({ message: 'Request already accepted or completed' });
