...
import { User, FoodRequest, UserRole, FoodRequestStatus } from '../db.ts';
...
export const updateVolunteerStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== UserRole.VOLUNTEER) return res.status(400).json({ message: 'User is not a volunteer' });
